...
with TestClient(app) as client:
    ...
    for task_name in input_dims:
        ...
        print(f"POST /predict/{task_name} (bad length) -> {r.status_code} (correctly rejected)")

        # /explain/{task}
        top_k = 5
        r = client.post(f"/explain/{task_name}", json={"features": X_sample}, params={"top_k": top_k})
        assert r.status_code == 200, r.text
        explanation = r.json()
        print(f"POST /explain/{task_name} -> explainer={explanation['explainer_used']}, "
        ...
        assert all("feature" in c and "shap_value" in c for c in explanation["top_contributions"])
