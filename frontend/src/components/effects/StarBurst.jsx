...
/**
 * StarBurst — a radial burst of light emanating from a focal point on a
 * transparent canvas. `starCount` evenly-spaced spokes radiate outward;
 * along each spoke a column of small pulses travels from the center
 * outward, twinkling as it goes, with a soft glowing core at the center.
 *
 * Adapted from the Originkit Framer component: background is transparent
 * (clearRect, not an opaque black fill) so it composites over existing
 * page content instead of requiring a full-bleed black canvas, and the
 * defaults are tuned down (fewer spokes, lower opacity) for use as a
 * contained accent rather than a hero-filling effect.
 */
export default function StarBurst(props) {
  const {
    speed = 6,
    starCount = 28,
    color = "#E7A23A",
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < nSpokes; i++) {
      ...
      spokeCos[i] = Math.cos(baseAngle + jitter);
      spokeSin[i] = Math.sin(baseAngle + jitter);
    }
