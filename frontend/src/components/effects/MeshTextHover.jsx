// Mesh Text Hover — Originkit, adapted from the Framer TS source to plain React/JSX.
// Logic ported as-is; only type annotations and the "use client" directive were stripped.
...
export default function MeshTextHover(props) {
  ...
  const fontSize = toNum(font?.fontSize, 180);
  const fontWeight = toNum(font?.fontWeight, variantToWeight(fontVariant));
  const fontStyle =
    typeof font?.fontStyle === "string" ? font.fontStyle : variantIsItalic(fontVariant) ? "italic" : "normal";
  ...
  return (
    <div ref={wrapperRef} style={{ position: "relative", width: "100%", height: "100%", overflow: "hidden", userSelect: "none" }}>
...
const COMPONENT_DEFAULTS = {
  text: "MESH",
  color: "#ffffff",
  font: { fontFamily: "Inter", variant: "Bold", fontSize: 180, lineHeight: "1em" },
  colorSplit: true,
