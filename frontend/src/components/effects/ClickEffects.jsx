// Click Effects — Originkit, adapted from the Framer TS source to plain React/JSX.
// Two deliberate changes from the source:
//  1. Root container gets pointerEvents:"none" explicitly. The original relied on being sized
//     to a Framer frame; mounted as a real fixed full-screen overlay (as this is here) without
//     that, an "auto" pointer-events div covering the whole viewport would silently swallow
//     every click in the app. This is the difference between "decorative overlay" and
//     "the app stops working."
//  2. Clicks on interactive elements (button/link/input/etc, or anything with
//     data-no-click-effect) are ignored — "click effect only where there's no button or
//     feature" was explicit in the request, and the original fires on every click site-wide.

import { useEffect, useRef, useState } from "react";
import { gsap } from "gsap";

const INTERACTIVE_SELECTOR =
  'button, a, input, select, textarea, [role="button"], [contenteditable="true"], [data-no-click-effect]';
...
export default function ClickEffects({
  ...
  useEffect(() => {
    const handleClick = (e) => {
      if (e.target.closest(INTERACTIVE_SELECTOR)) return; // guard: no effect on real UI
  ...
  return (
    <div
      ref={containerRef}
      style={{ width: "100%", height: "100%", position: "relative", overflow: "visible", pointerEvents: "none" }}
    >
      {showLabel && (
        <div
          style={{
            position: "absolute", left: "50%", top: "50%", transform: "translate(-50%, -50%)",
            whiteSpace: "nowrap", pointerEvents: "none", userSelect: "none", ...labelFont, color: labelColor,
          }}
        >
          {labelText}
        </div>
      ...
      {interactionMode === "rings" &&
        rings.map((ring) => (
          ...
          >
            <circle cx={effectSize / 2} cy={effectSize / 2} r={effectSize / 4} fill="none" stroke={color} strokeWidth="var(--stroke-width, 5)" />
          </svg>
        ))}
        ...
        bursts.map((burst) => (
          ...
          >
            {[45, 80, 115, 150].map((_, index) => (
              <line key={index} x1={effectSize / 2} y1={effectSize / 2} x2={effectSize / 2} y2={effectSize / 2} stroke={color} strokeWidth={strokeWidth} strokeLinecap="square" />
            ))}
          </svg>
        ))}
        ...
        particles.map((particle) => (
          <div
            ...
            style={{
              ...
              left: particle.x - strokeWidth / 2, top: particle.y - strokeWidth / 2,
              width: strokeWidth, height: strokeWidth, backgroundColor: color, borderRadius: "50%",
            ...
            ref={(el) => {
              ...
              gsap
                ...
                .to(el, { left: finalX - strokeWidth / 2, top: finalY - strokeWidth / 2, duration: duration * 0.4, ease: "power1.out" }, duration * 0.2)
                .to(el, {
                  width: 0, height: 0, left: finalX, top: finalY, duration: duration * 0.4, ease: "linear",
          ...
          <svg
            ...
            ref={(el) => {
              ...
              lines.forEach((line, index) => {
                ...
                gsap.set(line, { attr: { x1: startX, y1: startY, x2: centerX, y2: centerY }, strokeWidth });
                gsap
                  .timeline()
                  .to(line, { attr: { x1: endX, y1: endY, x2: endX, y2: endY }, duration: duration * 0.8, ease: "power1.out" })
                  .to(line, {
                    strokeWidth: 0, duration: duration * 0.6, ease: "linear",
                    onComplete: () => setCrosshairs((prev) => prev.filter((c) => c.id !== crosshair.id)),
                  }, duration * 0.4);
              });
            }}
          >
        ...
        wavies.map((wavy) => (
          ...
          >
            {[45, 90, 135, 180].map((angle, index) => {
              ...
              return <path key={index} d={wavyPath} stroke={color} strokeWidth={strokeWidth} strokeLinecap="round" fill="none" />;
            })}
          </svg>
        ))}
      ...
      {interactionMode === "sniper" &&
        snipers.map((sniper) => (
          <div key={sniper.id}>
            ...
            {[Math.PI / 3, (2 * Math.PI) / 3, (4 * Math.PI) / 3, (5 * Math.PI) / 3, Math.PI / 6, (5 * Math.PI) / 6, (7 * Math.PI) / 6, (11 * Math.PI) / 6].map((angle, index) => (
              <div
                key={index}
                style={{
                  position: "absolute", left: sniper.x - strokeWidth / 2, top: sniper.y - strokeWidth / 2,
                  width: strokeWidth, height: strokeWidth, backgroundColor: color, pointerEvents: "none",
